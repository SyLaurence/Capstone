    ...
    @if(session()->get('level') == 0)
          @section ('title')
          Admin | Offenses Record
          @endsection
        @else
          @section ('title')
          HR Staff | Offenses Record
          @endsection
        @endif
        @section ('pageContent')
        <!-- page content -->
                        ...
                        <table id="itemTable" class="table table-striped jambo_table bulk_action">
                          <thead>
                            <tr class="headings">
                              ...
                              <th class="column-title">Offense</th>
                                ...
                                <div class="modal-body">
                                    <div>
                                        ...
                                        <div class="row">
                                          ...
                                          <div class="col-md-8">
                                            <select name="txtOff" id="txtOff" class="form-control" required hidden>
          ...
          $.ajax({
            ...
            success: function(data){
                for(var ctr = 0; ctr< data.length; ctr++){
                  if(type == "All"){
                    $('#itemTable thead').append('<tr class=""><th>Name</th><th>Date and Time Occured</th><th>Offense</th><th>Level</th><th>Comment</th></tr>');
                    $('#itemTable tbody').append('<tr class=""><td>'+data[ctr].user+'</td><td>'+data[ctr].dt+'</td><td>'+data[ctr].off+'</td><td>'+data[ctr].lev+'</td><td>'+data[ctr].com+'</td></tr>');
                  } else {
                    $('#itemTable thead').append('<tr class=""><th>Name</th><th>Date and Time Occured</th><th>Offense</th><th>Comment</th></tr>');
                    
                    $('#itemTable tbody').append('<tr class=""><td>'+data[ctr].user+'</td><td>'+data[ctr].dt+'</td><td>'+data[ctr].off+'</td><td>'+data[ctr].com+'</td></tr>'); // Add
                  }
    ...
    <script type="text/javascript">
      ...
      function add(){
        ...
        $.ajax({
          url: '/Offense/addrecord/add',
          type: 'get',
          data: {
            'comment': document.getElementById('txtComment').value,
            'offid': document.getElementById('txtOff').value,
